...
class Settings(BaseSettings):
    ...
    def sync_database_url(self) -> str:
        ...
        Используется для применения миграций в Alembic.
        В проекте используется асинхронный доступ к БД в FastAPI,
        однако Alembic является синхронным инструментом миграций,
        поэтому для него применяется отдельный синхронный engine
        с драйвером psycopg2, что соответствует best practice.
        """
