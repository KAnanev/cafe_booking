...
class Dish(TimestampMixin, ActiveMixin, Base):
    ...
    name: Mapped[str] = mapped_column(String(DISH_NAME_MAX_LENGTH), nullable=False)
    description: Mapped[str | None] = mapped_column(String(DISH_DESCRIPTION_MAX_LENGTH), nullable=True)
    photo_id: Mapped[str | None] = mapped_column(
        nullable=True,
    )
    price: Mapped[float] = mapped_column(Numeric(10, 2), CheckConstraint('price >= 0'), nullable=False)
