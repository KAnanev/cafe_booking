...
def build_dish_response(dish) -> DishRead:
...
async def create_dish(
    ...
    """Создать новое блюдо в системе и привязать его к указанным кафе."""
...
async def get_dish(
    ...
    if not dish.is_active:
         ...
         if not is_admin_or_manager:
             raise HTTPException(status.HTTP_404_NOT_FOUND, 'Блюдо не найдено или не активно.')
